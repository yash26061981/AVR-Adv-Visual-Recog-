...
public class FineTuneClustering {
    ...
    private Raster inputImageRaster;

    public FineTuneClustering(BufferedImage h) {
        imageUtils = new ImageUtils();
        this.inputBufferedImage = h;
        inputImageRaster = inputBufferedImage.getRaster();
    }
    ...
    public List<CombinedImage> discardUnwantedPatches(double[][] labelImage) {
        ...
        for(int i=0;i<totalLabels.length;i++) {
            ...
            if (xmax >= inputImageRaster.getWidth()){
                xmax = inputImageRaster.getWidth()-1;
            }
            if (xmin <0)xmin = 0;
            ...
            if (ymax >= inputImageRaster.getHeight()){
                ymax = inputImageRaster.getHeight()-1;
            }
            if (ymin<0) ymin = 0;
            ...
            for(int ii=xmin;ii<=xmax;ii++) {
                for(int jj=ymin;jj<=ymax;jj++) {
                    charImage[newRow][newCol] = inputImageRaster.getSample(ii,jj,0);
                    newCol++;
                }
                newRow++;
                newCol = 0;
