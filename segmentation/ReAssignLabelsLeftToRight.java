...
public class ReAssignLabelsLeftToRight {
    ...
    public double[][] reAssignLabels(double[][] labelImage){
        ...
        int[] midLine = getMidLineForRearrangingLabels(labelImage);
        /*if (isSquarePlate){
            midLine = new int[2];
    ...
    private int[] getMidLineForRearrangingLabels(double[][] labelImage)
    {
        if (isSquarePlate){
            int blockSize = 5;
            int rows = labelImage[0].length;
            int totalBlocks = rows/blockSize;
            double[] blobsUpperLine = new double[totalBlocks/2];
            double[] blobsLowerLine = new double[totalBlocks/2];
            boolean isRow = true;
            int[] limit = {-1};
            for(int indx = 0; indx < totalBlocks/2; indx++){
                double[] midScanLine = imageUtils.getAllElementOfRowCol(labelImage, isRow, indx*blockSize, limit);
                blobsUpperLine[indx] = (imageUtils.findUnique(midScanLine)).length;

                midScanLine = imageUtils.getAllElementOfRowCol(labelImage, isRow, ((indx*blockSize) +1 + totalBlocks/2), limit);
                blobsLowerLine[indx] = (imageUtils.findUnique(midScanLine)).length;
            }
            int[] maxValIdUpper = imageUtils.findIndx(blobsUpperLine, "==", imageUtils.findMaximum(blobsUpperLine));
            int[] maxValIdLower = imageUtils.findIndx(blobsLowerLine, "==", imageUtils.findMaximum(blobsLowerLine));

            int[] midLine = new int[2];
            midLine[0] = (int)(imageUtils.sumOfArray(imageUtils.getDoubleCastArray(maxValIdUpper)) / maxValIdUpper.length)*blockSize;
            midLine[1] = (int)(imageUtils.sumOfArray(imageUtils.getDoubleCastArray(maxValIdLower)) / maxValIdLower.length)*blockSize;
            return midLine;
        }
        else {
            int blockSize = 10;
            int rows = labelImage[0].length;
            int totalBlocks = rows/blockSize;
            double[] blobsPerLine = new double[totalBlocks];
            boolean isRow = true;
            int[] limit = {-1};
            for(int indx = 0; indx < totalBlocks; indx++){
                double[] midScanLine = imageUtils.getAllElementOfRowCol(labelImage, isRow, indx*blockSize, limit);
                blobsPerLine[indx] = (imageUtils.findUnique(midScanLine)).length;
            }
            int[] maxValId = imageUtils.findIndx(blobsPerLine, "==", imageUtils.findMaximum(blobsPerLine));

            int[] midLine = new int[1];
            midLine[0] = (int)(imageUtils.sumOfArray(imageUtils.getDoubleCastArray(maxValId)) / maxValId.length) * blockSize;
            return midLine;
        }
