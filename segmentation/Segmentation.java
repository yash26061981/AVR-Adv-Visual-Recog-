...
import com.avanseus.avr.misc.MST;
...
public class Segmentation {
    ...
    public HashMap<Integer,int[][]> SegmentUsingConnectedComponentAnalysis(BufferedImage thresholdBmp){
        ...
        BufferedImage negateThresholdBmp;
        if(AVRConstants.USE_TATA_LOCALISED_BMP)
            negateThresholdBmp = thresholdBmp;
        else
            negateThresholdBmp = imageUtils.negateBitMap(thresholdBmp);

        double[][] labelImage = cca.findConnectedComponent(negateThresholdBmp);
        imageUtils.getVisibleImageFromBufferedImageOfLogicalOnes(imageUtils.getBufferedImageFrom2D(labelImage), "labelled");
        ...
        if(AVRConstants.USE_TATA_LOCALISED_BMP) {
            labelImage = RejectBiggerBlobsFromBMP(labelImage);
            imageUtils.getVisibleImageFromBufferedImageOfLogicalOnes(imageUtils.getBufferedImageFrom2D(labelImage), "label_deleted");
            FineTuneCropping fineTuneCropping = new FineTuneCropping();
            labelImage = fineTuneCropping.getMorphologicalCroppedBMPImage(labelImage);
            //imageUtils.getVisibleImageFromBufferedImageOfLogicalOnes(imageUtils.getBufferedImageFrom2D(labelImage), "label_cropped");
        }
        else
        ...
        labelImage = RejectDistantLabels(labelImage);
    ...
    private double[][] RejectDistantLabels(double[][] labelImage){
        double[] totalLabels = imageUtils.findUniqueIn2D(labelImage);
        if (totalLabels[0] == 0)
            totalLabels = imageUtils.shiftArrayLeft(totalLabels);
        double[] xCent = new double[totalLabels.length];
        double[] yCent = new double[totalLabels.length];
        for(int indx  = 0; indx < totalLabels.length; indx++){
            int[][] indxColRow = imageUtils.findIndxIn2D(labelImage,"==",totalLabels[indx]);
            int[] limit = {-1};
            double[] labelCols = imageUtils.getAllElementOfRowCol(imageUtils.getDoubleCastArray(indxColRow),true,0,limit);
            double[] labelRows = imageUtils.getAllElementOfRowCol(imageUtils.getDoubleCastArray(indxColRow),true,1,limit);
            int xmax = (int)(imageUtils.findMaximum(labelRows));
            int ymax = (int)(imageUtils.findMaximum(labelCols));
            int ymin = (int)imageUtils.findMinimum(labelCols);
            int xmin = (int)imageUtils.findMinimum(labelRows);
            xCent[indx] = ((double)(xmax+xmin))/2;
            yCent[indx] = ((double)(ymax+ymin))/2;
        }
        double[] choosenLabels;
        if(false) {
            double[] ydiff = imageUtils.findDifference(yCent);
            double[] xdiff = imageUtils.findDifference(xCent);
            MST mstxDiff = new MST(xdiff);
            MST mstyDiff = new MST(ydiff);
            double[] choosenXLabels = imageUtils.getDoubleCastArray(mstxDiff.filterComponents());
            double[] choosenYLabels = imageUtils.getDoubleCastArray(mstyDiff.filterComponents());
            if ( choosenXLabels.length < choosenYLabels.length ) {
                choosenLabels = choosenXLabels;
            } else {
                choosenLabels = choosenYLabels;
            }
        }else{
            double[] ydiff = imageUtils.findDifference(yCent);
            MST mstyDiff = new MST(ydiff);
        ...
        for(int indx  = 0; indx < totalLabels.length; indx++){
            ...
            double[] labelCols = imageUtils.getAllElementOfRowCol(imageUtils.getDoubleCastArray(indxColRow),true,0,limit);
            double[] labelRows = imageUtils.getAllElementOfRowCol(imageUtils.getDoubleCastArray(indxColRow),true,1,limit);
            if (false == imageUtils.isMember(totalLabels[indx],choosenLabels)){
                for (int indx1 = 0; indx1 < labelCols.length; indx1++){
    ...
    private double[][] RejectBiggerBlobsFromBMP(double[][] labelImage){
        double[] totalLabels = imageUtils.findUniqueIn2D(labelImage);
        if (totalLabels[0] == 0)
            totalLabels = imageUtils.shiftArrayLeft(totalLabels);
        ...
        totalLabels = imageUtils.findUniqueIn2D(labelImage);
        if (totalLabels[0] == 0)
            totalLabels = imageUtils.shiftArrayLeft(totalLabels);
        rowMean /= totalLabels.length;
        colMean /= totalLabels.length;
        for(int indx  = 0; indx < totalLabels.length; indx++){
            int[][] indxColRow = imageUtils.findIndxIn2D(labelImage,"==",totalLabels[indx]);
            int[] limit = {-1};
            double[] labelCols = imageUtils.getAllElementOfRowCol(imageUtils.getDoubleCastArray(indxColRow),true,0,limit);
            ...
            if (xSize > 2*rowMean || xSize < 0.5*rowMean || ySize > 2.5*colMean || ySize < 0.5*colMean){
                for (int indx1 = 0; indx1 < labelCols.length; indx1++){
                    labelImage[(int)labelCols[indx1]][(int)labelRows[indx1]] = 0;
                }
            }
