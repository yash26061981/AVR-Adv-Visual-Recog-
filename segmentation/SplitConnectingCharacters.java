...
public class SplitConnectingCharacters {
    ...
    public HashMap<Integer, int[][]> splitMergedCharacters(){
        ...
        if(AVRConstants.USE_TATA_LOCALISED_BMP)
            mergedIndx = imageUtils.findIndx(imageUtils.divideElementWise1D(colPair, rowPair), ">", 0.9);
        else
            mergedIndx = imageUtils.findIndx(imageUtils.divideElementWise1D(colPair, rowPair), ">", AVRConstants.NOT_MERGED_VALID_CHAR_AR_LIMIT);
    ...
    private HashMap<Integer, int[][]> splitCharacters(int[] mergedIndx){
        ...
        for(int indx = 0; indx < mergedIndx.length; indx++){
            int[][] mergedChar = null;
            if(AVRConstants.USE_TATA_LOCALISED_BMP) {
                int[][] mergedCharacter = characterLabels.get(mergedIndx[indx]);
                MorphologicalOperations morphologicalOperations = new MorphologicalOperations();
                double[][] se = morphologicalOperations.getStructuringElement("diamond", 2);
                BufferedImage erodedImage = morphologicalOperations.applyMorphologicalErosion(
                        morphologicalOperations.applyMorphologicalErosion(imageUtils.getBufferedImageFrom2D(mergedCharacter), se), se);
                imageUtils.getVisibleImageFromBufferedImageOfLogicalOnes(erodedImage, "connected_eroded_character");
                imageUtils.getVisibleImageFromBufferedImageOfLogicalOnes(imageUtils.getBufferedImageFrom2D(mergedCharacter), "connected_character");
                mergedChar = imageUtils.get2DImageInteger(erodedImage);
            }
            else
                mergedChar = characterLabels.get(mergedIndx[indx]);
    ...
    private int[] deleteCornerIds(int[] oldId, int col){
        ...
        if(newId != null){
            double[] diff1 = imageUtils.findDifference(imageUtils.getDoubleCastArray(newId));
            int[] diff2 = imageUtils.findIndx(diff1 , ">", 4.0);
            if(diff2.length == 0){
                int[] newId1 = new int[1];
                newId1[0] = newId[newId.length/2];
                return newId1;
            }
        }
        return newId;
