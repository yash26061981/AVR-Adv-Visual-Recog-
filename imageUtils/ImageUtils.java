...
import com.avanseus.avr.driver.DriverTataSegmentation;
...
public class ImageUtils {
    ...
    public BufferedImage getLogicalBufferedImageFrom2DLabelImage(double[][] inImage){
        ...
        for(int col = 0; col < inImage.length; col++){
            for(int row = 0; row < inImage[0].length; row++){
                double pixelValue = 0;
                if(inImage[col][row] > 0)
                    pixelValue = 1;
                writableRaster.setSample(col,row,0,pixelValue);
            }
        }
        return outImage;
    }
    ...
    public void getVisibleImageFromBufferedImageOfLogicalOnes(BufferedImage bufferedImage, String destFileName) {
        ...
        File outputDir = new File(DriverTataSegmentation.destDir+File.separator+destFileName+".jpg");
        try {
    ...
    public double[] findMaximumValueId(double[] inArr){
        double max = 0;
        int maxIndx = 0;
        double[] valId = new double[2];
        for(int indx=0; indx < inArr.length; indx++){
            if (inArr[indx] > max) {
                max = inArr[indx];
                maxIndx = indx;
        ...
        valId[0] = max; valId[1] = maxIndx;
        return valId;
    }
    public double[] findDifference(double[] inArr){
        double[] outArr = new double[inArr.length - 1];
        for(int indx = 1; indx < inArr.length; indx++){
            outArr[indx-1] = inArr[indx] - inArr[indx-1];
        }
        return outArr;
    }
