...
public class MorphologicalOperations {
    ...
    public double[][] getStructuringElement(String shape, int size){
        int newSz = 2*size-1;
        BufferedImage inImage = new BufferedImage(newSz,newSz,BufferedImage.TYPE_BYTE_GRAY);
        double[][] onesImage = imageUtils.getOnesImage(inImage);
        int origin = newSz/2;
        switch (shape) {
            case "disk":
                for (int r=0; r <onesImage.length; r++){
                    for (int c=0; c < onesImage[0].length; c++){
                        int euclidDist = (int) Math.sqrt(Math.pow((origin - r),2) + Math.pow((origin - c),2));
                        if(euclidDist > origin)
                            onesImage[r][c] = 0;
                    }
                }
                break;
            ...
            case "diamond":
                ...
                for(int x=0; x<origin; x++){
                    for(int y = 0, z = onesImage[0].length-1; y< origin-dsize && z >origin+dsize; y++,z--) {
                        onesImage[x][y] = 0;
                        onesImage[x][z] = 0;
                        onesImage[onesImage.length -1- x][y] = 0;
                        onesImage[onesImage.length -1- x][z] = 0;
                    }
                    dsize++;
                }
                break;
    ...
    static public void main(String args[]) throws Exception
    {
        ...
        double [][] stelement = morphologicalOperations.getStructuringElement("square",5);
    }
}
